(ns global-places-recommendation.selenium.selenium-test
  (:require [clj-webdriver.taxi :as tx]))

(defn start-selenium-session
  "Start selenium session and open browser"
  []
  (tx/set-driver! {:browser :firefox} "http://localhost:3000/login")
)
...
(defn login
  ...
  []
  (tx/input-text "#username" "user1")
  (tx/input-text "#password" "pass.1")
  (tx/submit "#password")
)
